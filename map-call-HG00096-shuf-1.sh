# whole genome HG00096 shuffled order - run 1
...
for i in `echo $FASTQ`;
do
	bwa mem -t $THREADS $REF $i\-shuf_1.fq.gz $i\-shuf_2.fq.gz  | samtools view -@ $THREADS -S -b -u - | samtools sort -@ $THREADS -m $MAXMEM -  tmp.$BAMFILE.$i;
done
